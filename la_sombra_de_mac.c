...
/*
	 Funcion que busca la puerta en un vector de herramientas y devuelve su coordenada 
*/
coordenada_t buscar_posicion_puerta(elemento_t herramientas[MAX_ELEMENTOS],int tope_herramientas){
	coordenada_t buscada;
	for(int i=0;i<tope_herramientas;i++){
		if(herramientas[i].tipo==PUERTA){
			buscada=herramientas[i].coordenada;
		}
	}
	return buscada;
}
/*
* Funcion que calcula la distancia manhattan entre dos coordenadas
*/
...
/*
	Funcion que revisa que una posicion este en el area de efecto de una vela , diagonalmente
*/
...
Funcion que revisa que en la siguiende posicion, dependiendo de hacia donde se mueva , hay una escalera o no
*/
...
/*	Funcion recursiva que se llama en caso de cruzarse con una escalera, devuelve la cantidad de "PASOS", que va a dar MAC,
 dependiendo de cuantas escaleras seguidas detecte, o en caso de cruzarse con una pared luego de la X cantidad de escaleras
...
void inicializar_niveles(juego_t* juego){
	...
	for(int o=POS_NIVEL_1;o<MAX_NIVELES;o++){
		if(o==POS_NIVEL_1){
			(*juego).niveles[o].numero_nivel=NIVEL_1;
			velas=VELAS_NIVEL_1;
			pozos=POZOS_NIVEL_1;
			...
			(*juego).niveles[o].numero_nivel=NIVEL_2;
			velas=VELAS_NIVEL_2;
			pozos=POZOS_NIVEL_2;
			...
			(*juego).niveles[o].numero_nivel=NIVEL_3;
			velas=VELAS_NIVEL_3;
			pozos=POZOS_NIVEL_3;
		...
		for(int i=0;i<(velas+pozos+interruptores+portales);i++){
			if(i<velas){
				...
				(*juego).niveles[o].obstaculos[i].coordenada=coordenada_aleatoria(MAX_FILAS,MAX_COLUMNAS);//ESTAS COORDENADAS SE TIENEN QUE BORRAR
...
int estado_nivel(juego_t juego){
	int nivel=(juego.nivel_actual)-1;
	coordenada_t posicion_puerta= buscar_posicion_puerta(juego.niveles[nivel].herramientas,juego.niveles[nivel].tope_herramientas);
	if(juego.nivel_actual==NIVEL_1){
		if((distancia_manhattan(juego.personaje.posicion,posicion_puerta)<=1)&&(distancia_manhattan(juego.sombra.posicion,posicion_puerta)<=1)&&(juego.sombra.esta_viva)){
			return GANADO;
		}
		else{
			return JUGANDO;
		...
		if(((distancia_manhattan(juego.personaje.posicion,posicion_puerta)<=1)&&(distancia_manhattan(juego.sombra.posicion,posicion_puerta)<=1))&&(juego.personaje.tiene_llave)&&(juego.sombra.esta_viva)){
			return GANADO;
		}
		else{
			return JUGANDO;
